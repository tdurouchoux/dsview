# ADR 0001: Entity Resolution classifier — model and prompt
...
Promote **`mistral-small-latest` with prompt `eval_configs/prompts/system_entity_resolution_v7.txt`**
to production for entity-resolution classification (`config/model.yaml`,
`prompts/system_entity_resolution.txt`).

## Consequences

- Lower per-call cost than the deprecated `mistral-medium-2508`, at matching measured quality.
- Error profile shifts to recall-favoring (occasional over-merge) rather than the prior balanced
  profile — worth a periodic spot-check of merged topics post-rollout.
- The hash-based split fix means all future eval/test comparisons are stable across labelling-set
  growth; re-running old configs will no longer silently drift due to split reshuffling.
- Further prompt tuning on this task looks close to a plateau for Mistral-family models (±0.01–0.02
  per iteration in later rounds); the next material gain, if needed, is more likely to come from a
  model change than another prompt revision.
