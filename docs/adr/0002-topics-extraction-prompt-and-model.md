...
# Topics extraction — prompt, model and output schema
...
*(The baseline table and its discussion below pre-date the dataset repair and `score_row` fix
described further down — comparable to each other, not to later tables. The logged KeyBERT/YAKE
runs also pre-date the MMR and `dedup_lim` settings now in the notebook.)*
...
* Exp 2 user prompt (name normalization) on `mistral-small-2603` at `temperature: 0` (chosen)
* Default prompt on `mistral-small-2603`
* Exp 1 — named-entity-first analysis on `mistral-small-2603`
* Exp 3 — Exp 2 + type definitions on `mistral-small-2603`
* Fable-authored variant — confidence-ranked output
* `mistral-medium-latest`, default prompt
* `mistral-medium-latest`, Exp 2 prompt
* Non-LLM baselines — YAKE, KeyBERT, GLiNER `gliner_medium-v2.1`
* Corpus-level topic modelling (LDA/NMF/BERTopic) — rejected without being run, wrong task shape

## Decision Outcome

Chosen option: "Exp 2 user prompt (`eval_configs/topics_exp2_user_prompt.txt`) on
`mistral-small-2603` at `temperature: 0`, with the `analysis` field kept first in `TopicList`",
because it delivers the best measured precision/recall/AP among the tested prompt variants for
this model (see Pros and Cons below), while the system prompt stays at its default since no
variant beat it.

Promoted to `prompts/user_topics_extraction.txt`.

### Consequences

* Good, because the expected production effect is recall +0.09 at flat precision (the held-out
  test-split number), smaller than the larger eval-split gains reported below.
* Bad, because the prompt is not model-portable — it corrects a bias specific to
  `mistral-small-2603` and actively harms a better-calibrated model. Any model change must
  re-evaluate it. Comparing predicted `Concept` share against the ~40% ground truth is a fast
  diagnostic.
* Neutral, because `mistral-medium-latest` is the better *base* model but not worth its cost:
  small+Exp 2 beats medium+default on all three metrics. Worth revisiting if the prompt is ever
  retired.
* Neutral, because remaining headroom is in **long documents** (~0.18 precision below short
  ones); the untested hypothesis is cap-padding, ceiling ~+0.06 overall.
* Bad, because **native reasoning could not be evaluated**. Both models support
  `reasoning_effort`, defaulting to `"none"` (verified: `"high"` returns a `ThinkChunk` at ~40×
  completion tokens). It is unreachable here because `chat.parse` raises `TypeError` on the
  list-valued `content` reasoning returns (`mistralai/extra/struct_chat.py` handles only
  `str`/`None`); testing it needs a provider change in `dsview/model_utils/providers/mistral.py`.
  Reasoning also rejects greedy sampling unless `top_p` is explicitly 1 (API error 3054).
* Neutral, because all metrics are 25 eval / 16 test rows; differences under ~0.02 are noise, and
  every `mistral-medium-latest` result is single-run.
...
* Bad, because statistics alone (YAKE) effectively fail on this corpus.
* Neutral, because embeddings (KeyBERT) triple AP over YAKE but the metric rewards redundant
  near-duplicate names — a measured 3.3× precision inflation before MMR.
* Neutral, because the taxonomy (GLiNER) gives the best non-LLM recall but is entity-only,
  capping at the ~60% of labels that are named entities.
* Good, because even the unrefined LLM baseline (default prompt) already clears every non-LLM
  method on all three metrics.
...
Model comparison (eval split): `mistral-medium-latest` scores **0.65 / 0.61 / 0.58** on the
default prompt but **0.57 / 0.57 / 0.50** on Exp 2.
