...
@lru_cache(maxsize=1)
def get_extraction_index(ttl_hash: int | None = None) -> ExtractionIndex:
...
@lru_cache(maxsize=1)
def get_topics_index(ttl_hash: int | None = None) -> TopicsIndex:
...
@lru_cache(maxsize=1)
def get_graph(ttl_hash: int | None = None) -> ig.Graph:
...
def _build_type_filter(
    column: str, types: list[str] | None, valid_types: list[str]
) -> list[str] | None:
    """
...
def search_content(
    search_term: str,
    limit: int = 10,
    types: Annotated[list[str] | None, "Filter content by types"] = None,
) -> DsviewContentList:
    """
...
def search_topic(
    search_term: str,
    limit: int = 10,
    types: Annotated[list[str] | None, "The type of topic to search for"] = None,
) -> DsviewTopicList:
    """
